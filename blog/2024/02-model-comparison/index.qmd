...
The `C(model)[T.lr]` variable (i.e., setting `lr` to be encoded as 1 while `gbt` was encoded as 0) quantifies the average difference in squared errors (variance) between the linear regression and the boosting trees models (`{python} float(round(rols.params['C(model)[T.lr]'], 2))`).
This allows us to directly assess the difference in model performance, but since we get asymptotic inference too, we can further place uncertainty around that difference - enabling us to tell whether it is statistically significant or not (p-value `{python} float(round(rols.pvalues['C(model)[T.lr]'], 4))`), or at least what is a likely range for that error to be in the "population" of data from which the dataset came from (95% confidence intervals [`{python} float(round(rols.conf_int(0.05).loc['C(model)[T.lr]', 0], 1)), float(round(rols.conf_int(0.05).loc['C(model)[T.lr]', 1], 1))`]).
