...
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.5, random_state=rng.integers(1000))
...
```{python}
sqe = pd.concat({"lr": lr_res, "gbt": gbt_res}, names=["model", "id"]).rename("li")
sqe
```
...
TODO: find models/data that differ significantly?
