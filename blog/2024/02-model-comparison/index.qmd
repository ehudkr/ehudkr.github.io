...
You are now able to compare models with some notion of uncertainty. 

## Going further

### Going Bayesian
Now, I'm positive what I presented here is equivalent to what happens in R when you use `anova(mod1, mod2)` (ANOVA with two models). 
In R, you often specify a `test` parameter to `anova` (like `"Chisq"` for a likelihood ratio test).
However, this requires knowing the degrees of freedom the models have in order to properly set up the null distribution to draw p-values from. 
...
[^9]: Or any deep learning model, to that matter. It seems the classical statistical theory does not always correspond to observed evidence from these types of models.

Adopting a Bayesian framework will come with multiple strengths:

* First, it will allow us to focus on magnitude of difference and estimation instead of null-hypothesis significant testing [@kruschke2013bayesian]; this will extra important once we touch on non-inferiority tests.
...
It's a powerful tool to specify regression formula (like above for `statsmodels`) but then it will utilize `PyMC` behind the scenes to perform MCMC-based Bayesian estimation, and do lots of good stuff under the hood like specifying priors automatically etc.
...
Think of a clinical setting where you want to test a new treatment. 
This treatment may not reduce mortality more than the standard of care, but this doesn't mean it useless.
Maybe it can be taken at home once a month, instead of 3 times a week at the clinic and have much fewer side effects? 
...
If both treatment have _similar_ or _comparable_ mortality rate, but the newer one is cheaper/easier to administer, etc. wouldn't we prefer it over the standard of care?
...
Wouldn't we want to assess whether models have comparable performance and then select the model that is simpler/cheaper to train or has a small carbon footprint?
Is it really worth spending x100 more FLOPs for a point improvement of 0.1 in some metric, 
...
We can use the Region of Practical Equivalence (ROPE) to specify a region that is equivalent to the null, 
...
az.plot_posterior(
    ...
    ref_val=0, 
    rope=[-0.1*np.std(y_test), 0.1*np.std(y_test)], 
)
```
...
1. **Multiple datasets: assess overall performance over multiple datasets**.  
   By concatenating the individual-level errors from multiple datasets, but still examine the difference between models.
   In such case, however, the response might no longer be individual and identically distributed (IID), because they originate from the same models.
   Ignoring these correlations in observations and assuming they are IID might make our estimates overly confident, which is the counterproductive to our main goal of assessing uncertainty properly. 
...
2. **Multiple models: Compare multiple models**. 
   Extending the dummy encoding of the model to any other contrast encoding between models can be done in a straightforward way, and allow comparing multiple models simultaneously. 
...
4. **Many types of metrics**
   ...
   For example, a classification model can be evaluated using the log-loss in a similar way to how it was done above, 
...
3. **Proper modeling of the errors**. 
   ...
   If we want to respect that support, we should, instead, 
...
[^10]: There are frequentist mixed models, and these correlation structures might also be accounted for used Generalized Estimating Equations (GEE), but their uncertainties are not always trustworthy. Bayesian mixed models, however, still shine. 
