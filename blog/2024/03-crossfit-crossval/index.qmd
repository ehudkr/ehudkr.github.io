...
[^1]: This is similar to how [CV-TMLE](https://biostats.bepress.com/ucbbiostat/paper273/) estimates an effect using TMLE within each test data partition, 
while the is [an equivalent version](https://arxiv.org/abs/1811.04573) in which TMLE is simply fed with test-data partition predictions (and the TMLE procedure is ran just once). 
...
Figure @fig-fig-crossfit_crossval visualizes this process, in which data is partitioned into 5 folds, each assigned a unique color (left).
...
![](double_crossfit-1.SVG){#fig-double_crossfit}

<!-- ![](double_crossfit-2.SVG) -->
