...
while the is an equivalent version [@levy2018easy] in which TMLE is simply fed with test-data partition predictions (and the TMLE procedure is ran just once). 
...
@fig-double_crossfit[^3] modifies @fig-crossfit_crossval to demonstrate that now each training fold for $\pi_{-k}$ and $m_{-k}$ is have the size (with the other half reserved to the other nuisance model). 
