...
So how do we force the alignment of eligibility, treatment initiation, and follow-up into a proper time zero?
...
data from before it are the baseline covariates;
...
(most design might enforce participants to be treatment naive).
...
TODO:
person-time format (heirarchical)
if the covariates are 1 year back (or start of data)
covariates for the same individual may change because treatments add.
outcome (time to event shortens)
illustrate that with a figure showing a person timeline and a person-time table. Make sure there's 1) a covariate being included overtime, 2) a covariate being excluded over time, 3) show the time-to-event reduces at each time point
