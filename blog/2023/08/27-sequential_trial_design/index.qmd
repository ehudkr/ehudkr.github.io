...
There are two main approaches for meta analysis.
...
However, advances in data management and digital communication made it increasingly feasible to share the original data on individual participants and give rise to a second type of meta analysis: one that consolidates individual-level data, basically consolidating datasets (@riley2010meta, @lin2010relative).

Consequently, these two approaches are applicable to sequential trials analysis.
...
However, since we extract all the trials from the same database, using the same study design with the same covariates, treatment and outcomes, it seems even more appropriate to consolidate the raw data.
This post even hinted to that, and this, to the best of my knowledge, the only way sequential trials have been analyzied in the epidemiological literature.
...
They might apply a multilevel (hierarchical) models, partially pooling estimates across time. 
They should defintely use robust variance estimation, either by sandwhich (HC) estimates or bootstrap, to estimate stnadard errors since the records in the consolidated dataset are no longer independent but are repeated measures of the same pool of individuals.
...
I also mentioned further considerations are required in computing standard errors as the rows of this dataset are no longer independent. 
...
The granularity of the time-steps is not just predefined, but is baked into the earliest point in the analytic pipeline.
This means that every sensitivity analysis on this the size of this time step parameter requires to apply the entire pipeline from data extraction from the database. 

This is problematic as the time granularity has both computational and practicical implications. 
Ideally, the time-step size should not be small enough to avoid creating too redundant repeatitions.
Too small and the size of the expanded person-time dataset will be too large and incur heavy computational cost (but may also cause floating-point issues as some survival models multiply probabilities over time-steps, so too many time-steps may become numerically unstable).
...
This is computationally harder in sequential trials since the size of the time-steps is baked deep into the construction of the datasets.
